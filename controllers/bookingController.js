        ...
        line_items:[{ // info of what user is gonna purchase 
            price_data:{
                ...
                product_data: {
                            ...
                            images: [`${req.protocol}://${req.get('host')}/img/tours/${bookedTour.imageCover}`], //imgs need to be live ie: hosted somewhere.
                        },
            },
        quantity: 1,
    }],
...
const createBookingCheckOut=  async sessionData=>{ // here sessionData is the same session that we made in above getChkoutSesssion but returned to us from stripe by listening to
    ...
    const price= (sessionData.amount_total) / 100 // since stripe takes the min amount of that currency($=>pence) so there we conv. it but need $ here to make bookingDoc
