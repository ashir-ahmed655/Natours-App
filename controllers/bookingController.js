...
const createBookingCheckOut=  async sessionData=>{ // here sessionData is the same session that we made in above getChkoutSesssion but returned to us from stripe by listening to
    ...
    const price= (sessionData.line_items[0].price_data.unit_amount) /100 // since stripe takes the min amount of that currency($=>pence) so there we conv. it but need $ here to make bookingDoc
